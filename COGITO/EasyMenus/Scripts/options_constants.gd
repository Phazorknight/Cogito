extends Node
class_name OptionsConstants

const sfx_bus_name = "SFX"
const music_bus_name = "Music" 
const config_file_name = "user://options.cfg"

const section_name = "Options" 
# Auido options
const sfx_volume_key_name = "sfx_volume"
const music_volume_key_name = "music_volume" 
# Graphics options
const windowmode_key_name = "window_mode"
const resolution_index_key_name = "resolution_index"
const render_scale_key = "render_scale"
const gui_scale_key = "gui_scale"
const vsync_key = "vsync"
const msaa_2d_key = "msaa_2d" 
const msaa_3d_key = "msaa_3d"
const brightness_key = "brightness"
const contrast_key = "contrast"
# Video options
const limit_fps_key = "fps"
const shadow_size_key = "shadow_size"
const shadow_filter_key = "shadow_filter"
#const sdfgi_key = "sdfgi"
#const glow_key = "glow"
const ssao_key = "ssao"
const ss_reflections_key = "ss_reflections"
const ssil_key = "ssil"
const volumetric_fog_key = "volumetric_fog"
# Gameplay options
const invert_vertical_axis_key = "invert_vertical_axis"
const head_bobble_key = "head_bobble"
